...
class ModelTrainer:
    ...
    def prepare_feature_matrix(
        ...
        for lag in range(1, lag_years + 1):
            ...
            X_data = X_data.merge(
                lag_indicators, on=["area_code", "year"], how="inner"
            )
        ...
        feature_cols = [
            ...
            if col not in ["area_code", "year", indicator_name]
        ]
        ...
        if not feature_cols:
            logger.warning(
                f"No usable feature columns for {indicator_name} after row filtering"
            )
            return np.array([]), np.array([]), []
        
        if len(data) < 10:
            logger.warning(
                f"Not enough data for {indicator_name}: only {len(data)} samples"
            )
            return np.array([]), np.array([]), []
