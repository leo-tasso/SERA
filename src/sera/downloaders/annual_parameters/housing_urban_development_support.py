...
class HousingUrbanDevelopmentSupportDownloader:
    ...
    def download_housing_urban_development_support(
        ...
        if not required.issubset(df.columns):
            raise RuntimeError(
                "Unexpected ISTAT response format for housing_urban_development_support."
            )
        ...
        df_clean["housing_urban_development_support"] = pd.to_numeric(
            df_clean["housing_urban_development_support"], errors="coerce"
        )
    ...
    def save_housing_urban_development_support_csv(
        ...
        if output_path is None:
            ...
            output_path = (
                indicator_dir / f"housing_urban_development_support_raw_{start_year}_{end_year}.csv"
            )

        print(f"Downloading housing urban development support data ({start_year}-{end_year})...")
        df = self.download_housing_urban_development_support(
