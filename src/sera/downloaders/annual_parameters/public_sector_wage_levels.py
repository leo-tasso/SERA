...
class PublicSectorWageLevelsDownloader:
    ...
    def download_public_sector_wage_levels(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not isinstance(payload, list) or len(payload) < 2 or not isinstance(payload[1], list):
            raise RuntimeError("Unexpected World Bank response format for public_sector_wage_levels.")
        ...
        df_clean["public_sector_wage_levels"] = pd.to_numeric(df_clean["public_sector_wage_levels"], errors="coerce")
    ...
    def save_public_sector_wage_levels_csv(
        ...
        if output_path is None:
            ...
            output_path = indicator_dir / f"public_sector_wage_levels_raw_{start_year}_{end_year}.csv"
