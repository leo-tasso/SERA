"""agriculture support level annual input parameter downloader (ISTAT source)."""
...
class AgricultureSupportLevelDownloader:
    """Download annual agriculture_support_level proxy for Italy from ISTAT."""
    ...
    def __init__(self):
        ...
        self.table_mapping: dict[str, Any] = {
            "parameter": "agriculture_support_level",
            "source": {
                "provider": "ISTAT",
                "flow_id": self.flow_id,
                "dataset": "Economic results of farms - key data",
                "columns": {
                    "REF_AREA": "area_code",
                    "TIME_PERIOD": "year",
                    "OBS_VALUE": "agriculture_support_level",
                },
            },
            "project": {
                "entity": "policy",
                ...
                "notes": "Annual proxy derived from ISTAT farm economic results.",
            },
        }
    ...
    def download_agriculture_support_level(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not required.issubset(df.columns):
            raise RuntimeError("Unexpected ISTAT response format for agriculture_support_level.")

        if "FREQ" in df.columns and (df["FREQ"] == "A").any():
            df = df[df["FREQ"] == "A"]

        df_clean = df[["REF_AREA", "TIME_PERIOD", "OBS_VALUE"]].copy()
        df_clean.columns = ["area_code", "year", "agriculture_support_level"]
        ...
        df_clean = (
            df_clean.groupby(["area_code", "year"], as_index=False)["agriculture_support_level"]
            .mean()
    ...
    def save_agriculture_support_level_csv(
        self,
        ...
        print(f"✓ agriculture support level data saved: {output_path}")
