...
class SocialWelfareSpendingAllocationDownloader:
    ...
    def download_social_welfare_spending_allocation(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not required.issubset(df.columns):
            raise RuntimeError("Unexpected ISTAT response format for social_welfare_spending_allocation.")
        ...
        df_clean["social_welfare_spending_allocation"] = pd.to_numeric(df_clean["social_welfare_spending_allocation"], errors="coerce")
        df_clean = df_clean.dropna(subset=["year", "social_welfare_spending_allocation"])
        df_clean = df_clean[(df_clean["year"] >= start_year) & (df_clean["year"] <= end_year)]
        df_clean = (
            df_clean.groupby(["area_code", "year"], as_index=False)["social_welfare_spending_allocation"]
            .mean()
    ...
    def save_social_welfare_spending_allocation_csv(
        ...
        if output_path is None:
            ...
            output_path = indicator_dir / f"social_welfare_spending_allocation_raw_{start_year}_{end_year}.csv"

        print(f"Downloading social welfare spending allocation data ({start_year}-{end_year})...")
        df = self.download_social_welfare_spending_allocation(start_year=start_year, end_year=end_year)
