...
class BroadbandPenetrationDownloader:
    ...
    def download_broadband_penetration(
        ...
        df_clean["broadband_penetration"] = pd.to_numeric(
            df_clean["broadband_penetration"], errors="coerce"
        )
        df_clean = df_clean.dropna(subset=["year", "broadband_penetration"])
        df_clean = df_clean[(df_clean["year"] >= start_year) & (df_clean["year"] <= end_year)]
        return df_clean.sort_values(["area_code", "year"])

    def save_broadband_penetration_csv(
