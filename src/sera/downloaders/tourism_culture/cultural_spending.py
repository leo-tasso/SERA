...
class CulturalSpendingDownloader:
    ...
    def download_cultural_spending(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        df_clean["cultural_spending"] = pd.to_numeric(df_clean["cultural_spending"], errors="coerce")
        df_clean = df_clean.dropna(subset=["year", "cultural_spending"])
        df_clean = df_clean[(df_clean["year"] >= start_year) & (df_clean["year"] <= end_year)]
        return df_clean.sort_values(["area_code", "year"])

    def save_cultural_spending_csv(self, output_path: Optional[Path] = None, start_year: int = 2001, end_year: int = 2025) -> Path:
