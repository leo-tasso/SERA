"""Life expectancy indicator downloader (World Bank source)."""
...
class LifeExpectancyDownloader:
    """Download life expectancy at birth for Italy from the World Bank."""
    ...
    def __init__(self):
        ...
        self.table_mapping: dict[str, Any] = {
            ...
            "project": {
                ...
                "levels": ["national"],
                "notes": "Life expectancy at birth, total (years).",
            },
        }
    ...
    def download_life_expectancy(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not isinstance(payload, list) or len(payload) < 2:
            raise RuntimeError("Unexpected World Bank response format for life expectancy.")
    ...
    def save_life_expectancy_csv(
        ...
        if output_path is None:
            indicator_dir = get_indicator_data_dir("life_expectancy")
            output_path = indicator_dir / f"life_expectancy_raw_{start_year}_{end_year}.csv"

        print(f"Downloading life expectancy data ({start_year}-{end_year})...")
        df = self.download_life_expectancy(start_year=start_year, end_year=end_year)
        ...
        print(f"✓ Life expectancy data saved: {output_path}")
