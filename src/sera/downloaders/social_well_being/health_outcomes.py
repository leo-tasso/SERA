"""Health outcomes indicator downloader (World Bank source)."""
...
class HealthOutcomesDownloader:
    """Download infant mortality rate for Italy as a health outcomes indicator."""
        ...
        self.table_mapping: dict[str, Any] = {
            ...
            "source": {
                ...
                "columns": {
                    ...
                    "date": "year",
                    "value": "infant_mortality_per_1000",
                },
            },
            "project": {
                "entity": "geography",
                ...
                "levels": ["national"],
                "notes": "Health outcomes proxy: infant mortality rate (per 1,000 live births).",
            },
        }
    ...
    def download_health_outcomes(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not isinstance(payload, list) or len(payload) < 2:
            raise RuntimeError("Unexpected World Bank response format for health outcomes.")
    ...
    def save_health_outcomes_csv(
        ...
        if output_path is None:
            indicator_dir = get_indicator_data_dir("health_outcomes")
            output_path = indicator_dir / f"health_outcomes_raw_{start_year}_{end_year}.csv"

        print(f"Downloading health outcomes data ({start_year}-{end_year})...")
        df = self.download_health_outcomes(start_year=start_year, end_year=end_year)
        ...
        print(f"✓ Health outcomes data saved: {output_path}")
