...
class BroadbandSpeedReliabilityDownloader:
    ...
    def download_broadband_speed_reliability(
        ...
        if not isinstance(payload, list) or len(payload) < 2 or not isinstance(payload[1], list):
            raise RuntimeError(
                "Unexpected World Bank response format for broadband_speed_reliability."
            )
        ...
        df_clean["broadband_speed_reliability"] = pd.to_numeric(
            df_clean["broadband_speed_reliability"], errors="coerce"
        )
        df_clean = df_clean.dropna(subset=["year", "broadband_speed_reliability"])
        df_clean = df_clean[(df_clean["year"] >= start_year) & (df_clean["year"] <= end_year)]
        return df_clean.sort_values("year")

    def save_broadband_speed_reliability_csv(
        ...
        if output_path is None:
            ...
            output_path = (
                indicator_dir / f"broadband_speed_reliability_raw_{start_year}_{end_year}.csv"
            )
