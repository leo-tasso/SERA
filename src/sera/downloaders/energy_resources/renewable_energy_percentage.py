...
class RenewableEnergyPercentageDownloader:
    ...
    def download_renewable_energy_percentage(
        ...
        if not isinstance(payload, list) or len(payload) < 2 or not isinstance(payload[1], list):
            raise RuntimeError(
                "Unexpected World Bank response format for renewable_energy_percentage."
            )
        ...
        df_clean["renewable_energy_percentage"] = pd.to_numeric(
            df_clean["renewable_energy_percentage"], errors="coerce"
        )
        df_clean = df_clean.dropna(subset=["year", "renewable_energy_percentage"])
        df_clean = df_clean[(df_clean["year"] >= start_year) & (df_clean["year"] <= end_year)]
        return df_clean.sort_values("year")

    def save_renewable_energy_percentage_csv(
        ...
        if output_path is None:
            ...
            output_path = (
                indicator_dir / f"renewable_energy_percentage_raw_{start_year}_{end_year}.csv"
            )
