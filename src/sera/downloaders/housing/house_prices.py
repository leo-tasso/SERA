"""House prices indicator downloader (ISTAT source)."""
...
class HousePricesDownloader:
    """Download house prices index from ISTAT."""

    def __init__(self):
        self.client = IstatClient()
        self.flow_id = "143_497"
        self.table_mapping: dict[str, Any] = {
            ...
            "project": {
                ...
                "levels": ["national"],
                "notes": "ISTAT house price index for Italy (all dwellings).",
            },
        }
    ...
    def save_house_prices_csv(self, output_path: Optional[Path] = None, start_year: int = 2001, end_year: int = 2025) -> Path:
        ...
        print(f"Downloading house prices data ({start_year}-{end_year})...")
        df = self.download_house_prices(start_year=start_year, end_year=end_year)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(output_path, index=False, encoding="utf-8")
        mapping_path = self._save_table_mapping(output_path)
        print(f"✓ House prices data saved: {output_path}")
