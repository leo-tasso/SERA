...
class InfrastructureInvestmentDownloader:
    ...
    def download_infrastructure_investment(
        ...
        if not isinstance(payload, list) or len(payload) < 2 or not isinstance(payload[1], list):
            raise RuntimeError(
                "Unexpected World Bank response format for infrastructure investment."
            )
        ...
        df_clean["infrastructure_investment_pct_gdp"] = pd.to_numeric(
            df_clean["infrastructure_investment_pct_gdp"], errors="coerce"
        )
    ...
    def save_infrastructure_investment_csv(
        ...
        if output_path is None:
            ...
            output_path = (
                indicator_dir / f"infrastructure_investment_raw_{start_year}_{end_year}.csv"
            )
