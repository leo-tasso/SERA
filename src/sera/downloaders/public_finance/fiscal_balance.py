"""Fiscal balance indicator downloader (World Bank proxy source)."""
...
class FiscalBalanceDownloader:
    """Download fiscal balance proxy for Italy from World Bank."""
    ...
    def __init__(self):
        ...
        self.table_mapping: dict[str, Any] = {
            ...
            "source": {
                ...
                "indicator_name": "Net lending/borrowing (% of GDP)",
                "columns": {
                    "countryiso3code": "area_code",
                    "date": "year",
                    "value": "fiscal_balance_pct_gdp",
                },
            },
            "project": {
                "entity": "policy",
                ...
                "levels": ["national"],
                "notes": "Fiscal balance proxy from World Bank net lending/borrowing (% of GDP).",
            },
        }
    ...
    def download_fiscal_balance(self, start_year: int = 2001, end_year: int = 2025) -> pd.DataFrame:
        ...
        if not isinstance(payload, list) or len(payload) < 2 or not isinstance(payload[1], list):
            raise RuntimeError("Unexpected World Bank response format for fiscal balance.")
    ...
    def save_fiscal_balance_csv(self, output_path: Optional[Path] = None, start_year: int = 2001, end_year: int = 2025) -> Path:
        ...
        print(f"Downloading fiscal balance data ({start_year}-{end_year})...")
        df = self.download_fiscal_balance(start_year=start_year, end_year=end_year)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(output_path, index=False, encoding="utf-8")
        mapping_path = self._save_table_mapping(output_path)
        print(f"✓ Fiscal balance data saved: {output_path}")
