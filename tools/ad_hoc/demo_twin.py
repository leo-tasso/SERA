...
def demo_basic_training_and_simulation():
    ...
    try:
        ...
        initial_state = indicators_df[indicators_df["year"] == 2025].copy() if not indicators_df.empty else None
        ...
        for indicator in indicators.keys():
            if indicator in initial_state.columns:
                change = (next_state[indicator].mean() - initial_state[indicator].mean()) / initial_state[indicator].mean() * 100
...
def demo_scenario_comparison():
    ...
    try:
        ...
        print("-" * 60)
        comparison = AnalysisTools.compare_scenarios(
            results_a, results_b, "income", 2030
        )
