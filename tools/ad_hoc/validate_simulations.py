...
for label, filepath in output_files:
    ...
    try:
        ...
        is_valid = (
            len(indicator_cols) == len(expected_indicators)
            and unique_provinces == expected_provinces
            and nan_count == 0
            and inf_count == 0
            and area_code_nans == 0
        )
        ...
        results_summary.append({"horizon": label, "status": "[FILE_NOT_FOUND]"})
    except Exception as e:
        print(f"[ERROR] {e}")
        results_summary.append({"horizon": label, "status": f"[ERROR]"})
...
for row in results_summary:
    if "shape" in row:
        print(
            f"{row['horizon']:<12s} {row['status']:<12s} {str(row['shape']):<18s} {str(row['indicators'])+'/24':<12s} {str(row['provinces'])+'/110':<12s}"
        )
    else:
