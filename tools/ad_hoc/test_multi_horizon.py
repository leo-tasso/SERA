...
def load_or_train_models(model_path: Path = Path("twin_models_full.joblib")) -> ModelTrainer:
    ...
    if model_path.exists():
        ...
        try:
            ...
            print(f"Loaded {len(indicators_df)} indicator samples, {len(parameters_df)} parameter samples")
...
def create_scenario(
    ...
    parameter_cols = [
        "income_tax_rate", "corporate_tax_rate", "property_wealth_tax_rate", "vat_consumption_tax_rate",
        "healthcare_spending_allocation", "education_spending_allocation", "infrastructure_investment_allocation",
        "social_welfare_spending_allocation", "rd_innovation_incentives", "green_energy_environment_investment",
        "agriculture_support_level", "manufacturing_incentives", "tourism_support_level", "small_business_support",
        "immigration_policy_level", "regulatory_burden_level", "public_sector_wage_levels",
        "pension_retirement_spending", "housing_urban_development_support", "environmental_regulations_strictness",
    ]
    ...
    for year_idx in range(years):
        ...
        params_df["property_wealth_tax_rate"] = parameter_baselines["property_wealth_tax_rate"] * tax_multiplier
        params_df["vat_consumption_tax_rate"] = parameter_baselines["vat_consumption_tax_rate"] * tax_multiplier
...
def validate_results(
    ...
    positive_indicators = [
        "income", "gdp_per_capita", "population", "life_expectancy", 
        "school_enrollment", "employment_rate", "house_prices"
    ]
    ...
    if "poverty_rate" in results.columns:
        if (results["poverty_rate"] > 100).any():
            report["issues"].append(
                f"Poverty rate > 100%: max={results['poverty_rate'].max():.2f}"
            )
    
    # Compute stats per year for key indicators
    key_indicators = [
        "income", "unemployment_rate", "life_expectancy", "poverty_rate",
        "school_enrollment", "gdp_per_capita", "carbon_emissions", "renewable_energy_percentage"
    ]
            ...
            try:
                ...
                for year, stats in report["stats"].items():
                    ...
                    if "income_mean" in stats and stats["income_mean"] is not None:
                        print(f"      - Income: {stats['income_mean']:,.0f} ± {stats['income_std']:,.0f}")
                    if "unemployment_rate_mean" in stats and stats["unemployment_rate_mean"] is not None:
                        print(f"      - Unemployment: {stats['unemployment_rate_mean']:,.0f} ± {stats['unemployment_rate_std']:,.0f}")
                    if "life_expectancy_mean" in stats and stats["life_expectancy_mean"] is not None:
                        print(f"      - Life Expectancy: {stats['life_expectancy_mean']:.2f} ± {stats['life_expectancy_std']:.2f}")
                    if "poverty_rate_mean" in stats and stats["poverty_rate_mean"] is not None:
                        print(f"      - Poverty Rate: {stats['poverty_rate_mean']:.2f}% ± {stats['poverty_rate_std']:.2f}%")
