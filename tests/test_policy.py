...
def make_env(horizon=3, constraint_fn=None, reserve_pool=0.0):
    return RolloutEnv(
...
class TestRolloutEnv:
    def test_rollout_returns_four_tuple_with_expected_shapes(self):
        env = make_env(horizon=3)
        trajectory, gdp_series, allocations_by_year, final_reserve = env.rollout(
    ...
    def test_baseline_gdp_accumulates_lever_a(self):
        ...
        _t, gdp_series, _a, _r = env.rollout(BaselinePolicy(SPECS))
    ...
    def test_constraint_fn_receives_and_returns_reserve(self):
        ...
        _t, _g, _a, final_reserve = env.rollout(BaselinePolicy(SPECS))
...
class TestPolicies:
    ...
    def test_gdp_nn_decide_respects_bounds(self):
        env = make_env()
        policy = GdpMaximizerPolicy(SPECS, seed=1)
    ...
    def test_gdp_nn_is_deterministic_for_a_seed(self):
        env = make_env()
        a = GdpMaximizerPolicy(SPECS, seed=7).decide(make_state(), 0, env)
        b = GdpMaximizerPolicy(SPECS, seed=7).decide(make_state(), 0, env)
        assert a == b

    def test_gdp_nn_fit_improves_or_keeps_score(self):
        env = make_env(horizon=2)
        policy = GdpMaximizerPolicy(SPECS, seed=0)
    ...
    def test_fit_progress_callback_called(self):
        ...
        calls = []
        GdpMaximizerPolicy(SPECS, seed=0).fit(
...
class TestRegistry:
    def test_available_models_metadata(self):
        ...
        assert models["gdp_nn"]["trainable"] is True

    def test_build_policy_by_id(self):
        assert isinstance(build_policy("baseline", SPECS), BaselinePolicy)
        assert isinstance(build_policy("gdp_nn", SPECS), GdpMaximizerPolicy)

    def test_build_policy_unknown_id_falls_back_to_gdp_nn(self):
        policy = build_policy("does-not-exist", SPECS)
        assert isinstance(policy, GdpMaximizerPolicy)
